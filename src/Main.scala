...
class Main extends PortableApplication(2000, 1000) {
  ...
  override def onGraphicRender(g: GdxGraphics): Unit = {
    ...
    var v: Vector2 = p1.getLocalVelocity
    v.scl(10)
    v.setAngle(p1.playerBox.getBodyAngleDeg())
    v.rotate(-90)
    g.drawLine(p1.playerBox.getBodyPosition().x, p1.playerBox.getBodyPosition().y, p1.playerBox.getBodyPosition().x + v.x, p1.playerBox.getBodyPosition().y + v.y)
